...
void MiddlewareRouter::addRule(string name, unique_ptr<IMatcher> matcher, FactoryType factoryName) {
    Rule r;
    r.name = move(name);
    r.matcher = move(matcher);
    ...
    for (auto &r : rules_) {
        ...
        if (r.matcher->matches(req)) {
            if (!r.chainHead) {
                ...
                } else if (!(r.factoryName == FactoryType::Unknown)) {
...
void MiddlewareRouter::insertAfter(const string& ruleName, const string& existingHandlerName, unique_ptr<IHandler> handler) {
    ...
    if (!r->chainHead) {
        ...
        else if (!(r->factoryName == FactoryType::Unknown)) r->chainHead = ChainFactoryRegistry::instance().createChain(r->factoryName);
    }
